...
import { DUMMY_TASKS, DUMMY_RESULTS } from '../data/dummy';
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {result.skills.map((skill) => (
                  <span key={skill} className="px-3 py-1 rounded-full bg-primary-500/10 text-primary-300 text-xs font-medium border border-primary-500/20">
                    {skill}
                  </span>
                ))}
              </div>
                ...
                {result.section_feedback.map((sf) => (
                  <div key={sf.section} className="glass-light rounded-lg p-4">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className={`text-sm font-bold ${sf.score >= 80 ? 'text-success-400' : sf.score >= 60 ? 'text-warning-400' : 'text-danger-400'}`}>
                        {sf.score}/100
                      </span>
                    </div>
                    <div className="w-full h-1.5 rounded-full bg-surface-700 overflow-hidden">
                      <div
                        className={`h-full rounded-full transition-all duration-1000 ${sf.score >= 80 ? 'bg-success-400' : sf.score >= 60 ? 'bg-warning-400' : 'bg-danger-400'}`}
                    ...
                    {sf.strengths.length > 0 && (
                      <div className="mt-2">
                        {sf.strengths.map((s, i) => (
                          <p key={i} className="text-xs text-success-400/80 flex items-start gap-1.5 mt-1">
                            <span className="mt-0.5">✓</span> {s}
                          </p>
                        ))}
                      </div>
                    )}
                    {sf.weaknesses.length > 0 && (
                      <div className="mt-1.5">
                        {sf.weaknesses.map((w, i) => (
                          <p key={i} className="text-xs text-warning-400/80 flex items-start gap-1.5 mt-1">
              ...
              <div className="space-y-2">
                {result.suggestions.map((s, i) => (
                  <div key={i} className="flex items-start gap-3 text-sm text-surface-300">
                    <span className="w-5 h-5 rounded-full bg-primary-500/15 text-primary-400 flex items-center justify-center text-xs font-bold shrink-0 mt-0.5">{i + 1}</span>
                    {s}
                  </div>
                ))}
              </div>
            ...
            <div>
              ...
              <div className="glass-light rounded-lg p-4 space-y-2">
                {result.ats_tips.map((tip, i) => (
                  <p key={i} className="text-sm text-accent-400/90 flex items-start gap-2">
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {result.keywords_missing.map((kw) => (
                  <span key={kw} className="px-3 py-1 rounded-full bg-danger-500/10 text-danger-400 text-xs font-medium border border-danger-500/20">
                    {kw}
                  </span>
                ))}
              </div>
                ...
                {result.work_experience.map((we, i) => (
                  <div key={i} className="glass-light rounded-lg p-4">
                    <div className="flex items-start justify-between">
                      <div>
                        <p className="text-sm font-semibold text-surface-100">{we.title}</p>
                        <p className="text-sm text-primary-400">{we.company}</p>
                      </div>
                      ...
                      <ul className="mt-2 space-y-1">
                        {we.highlights.map((h, j) => (
                          <li key={j} className="text-xs text-surface-400 flex items-start gap-2">
                            <span className="text-surface-600 mt-0.5">•</span> {h}
                          </li>
                        ))}
                      </ul>
                    )}
...
export default function Dashboard() {
  ...
  const completedTasks = DUMMY_TASKS.filter((t) => t.status === 'completed');
  const avgScore = completedTasks.length > 0
    ? Math.round(completedTasks.reduce((sum, t) => sum + (DUMMY_RESULTS[t.id]?.overall_score ?? 0), 0) / completedTasks.length)
    : 0;
  const totalSkills = new Set(
    completedTasks.flatMap((t) => DUMMY_RESULTS[t.id]?.skills ?? [])
  ).size;
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <nav className="sticky top-0 z-40 glass border-b border-surface-700/50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          ...
          <div className="hidden sm:flex items-center gap-4">
            ...
            <button
              id="logout-button"
              onClick={logout}
              className="px-3 py-1.5 rounded-lg text-sm text-surface-400 hover:text-surface-200 hover:bg-surface-800 transition-all border-none cursor-pointer bg-transparent font-medium"
            >
              Sign out
            </button>
          </div>
          ...
          {/* Mobile menu button */}
          <button
            className="sm:hidden w-8 h-8 flex items-center justify-center rounded-lg hover:bg-surface-800 transition-colors border-none bg-transparent cursor-pointer text-surface-300"
        ...
        {mobileMenuOpen && (
          <div className="sm:hidden border-t border-surface-700/50 p-4 space-y-3 animate-slide-down">
            ...
            <button onClick={logout} className="w-full text-left px-3 py-2 rounded-lg text-sm text-danger-400 hover:bg-surface-800 transition-all border-none cursor-pointer bg-transparent">
              Sign out
            </button>
          </div>
        )}
      ...
      <main className="flex-1 max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8 animate-slide-up [animation-delay:0.1s]">
          ...
          />
          <StatCard
            icon={<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2" /></svg>}
            label="Skills Found"
            value={totalSkills}
        ...
        <div className="mb-8 animate-slide-up [animation-delay:0.15s]">
          <button
            ...
            onClick={() => alert('Upload functionality will be connected to the backend API')}
          >
            ...
            <table className="w-full">
              ...
              <tbody>
                {DUMMY_TASKS.map((task) => {
                  const result = DUMMY_RESULTS[task.id];
                  return (
                    <tr key={task.id} className="border-b border-surface-700/30 last:border-b-0 hover:bg-surface-800/30 transition-colors">
                      ...
                      <td className="px-5 py-4">
                        {result?.overall_score != null ? (
                          <ScoreRing score={result.overall_score} size={44} />
                        ) : (
                      ...
                      <td className="px-5 py-4">
                        <div className="flex flex-wrap gap-1 max-w-[200px]">
                          {(result?.skills ?? []).slice(0, 3).map((s) => (
                            <span key={s} className="text-xs px-2 py-0.5 rounded bg-surface-800 text-surface-300">{s}</span>
                          ))}
                          {(result?.skills?.length ?? 0) > 3 && (
                      ...
                      <td className="px-5 py-4 text-right">
                        {result ? (
                          <button
                            ...
                            className="text-xs font-medium text-primary-400 hover:text-primary-300 transition-colors bg-transparent border-none cursor-pointer"
                          >
                            View details →
                          </button>
                        ) : (
          ...
          <div className="md:hidden space-y-3">
            {DUMMY_TASKS.map((task) => {
              ...
              return (
                <div
                  key={task.id}
                  className="glass-light rounded-xl p-4 cursor-pointer hover:border-primary-500/20 transition-all"
                ...
                >
                  <div className="flex items-start justify-between mb-3">
                    ...
                    {result?.overall_score != null ? (
                      <ScoreRing score={result.overall_score} size={40} />
                    ) : (
      ...
      {/* ── Result Panel ────────────────────────────────── */}
      {selectedResult && (
        <ResultPanel result={selectedResult} onClose={() => setSelectedResult(null)} />
      )}
    </div>
  );
}
